...
struct kobject *properties_kobj_rmidev_hd;
...
/*	rmidev->sysfs_dir = kobject_create_and_add("rmidev",
	...
	properties_kobj_rmidev_hd = kobject_create_and_add("rmidev", properties_kobj_synap_hd);
	if (!properties_kobj_rmidev_hd) {
			dev_err(&rmidev->rmi4_data->i2c_client->dev,
					"%s: Failed to create sysfs directory\n",
					__func__);
			goto err_sysfs_dir;
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) {
		...
		retval = sysfs_create_file(properties_kobj_rmidev_hd, &attrs[attr_count].attr);
...
err_sysfs_attrs:
	for (attr_count--; attr_count >= 0; attr_count--) {
		sysfs_remove_file(properties_kobj_rmidev_hd, &attrs[attr_count].attr);
	}

	kobject_put(properties_kobj_rmidev_hd);
...
{
	synaptics_rmi4_new_function_hd(RMI_DEV, true,
			rmidev_init_device,
...
static void __exit rmidev_module_exit(void)
{
	...
	synaptics_rmi4_new_function_hd(RMI_DEV, false,
			rmidev_init_device,
			rmidev_remove_device,
			NULL);
