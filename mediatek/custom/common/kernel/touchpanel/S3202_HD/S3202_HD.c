...
#include "tpd_custom_s3202_hd.h"
#include "cust_gpio_usage.h"
#include "tpd.h"
#include "synaptics_dsx_rmi4_i2c_hd.h"
#include "SynaImage_hd.h"
...
const u16 touch_key_array_hd[] = { KEY_MENU, KEY_HOMEPAGE, KEY_BACK};
#define MAX_KEY_NUM ( sizeof( touch_key_array_hd )/sizeof( touch_key_array_hd[0] ) )
#endif
...
extern int synaptics_fw_updater_hd(unsigned char *fw_data);
extern int fwu_check_version(void);
//static u8 get_config_version(void);


static const struct i2c_device_id tpd_id[] = {{"S3202_HD",0},{}};
/* < DTS2012040603460 gkf61766 20120406 begin */
static unsigned short force[] = {0,(0x22<<1),I2C_CLIENT_END,I2C_CLIENT_END};
/* < DTS2012040603460 gkf61766 20120406 end */
static const unsigned short * const forces[] = { force, NULL };
//static struct i2c_client_address_data addr_data = { .forces = forces, };
static struct i2c_board_info __initdata i2c_tpd={ I2C_BOARD_INFO("S3202_HD", (0x70))};
...
#if 1
//glove patterns
static ssize_t synaptics_rmi4_glove_patterns_enables_show(struct device *dev,
                struct device_attribute *attr, char *buf);
	              
static ssize_t synaptics_rmi4_glove_patterns_enables_store(struct device *dev,
                struct device_attribute *attr, const char *buf, size_t count);

//firmware version
static ssize_t synaptics_rmi4_firmware_version_show(struct device *dev,
                struct device_attribute *attr, char *buf);
static ssize_t synaptics_rmi4_firmware_version_store(struct device *dev,
                struct device_attribute *attr, const char *buf, size_t count);
...
static struct kobj_attribute synaptics_glove_patterns_enables_attr = {
    .attr = {
        .name = "golve_patterns_enables",
        .mode = (S_IRUGO | S_IWUGO),
    },
    .show = &synaptics_rmi4_glove_patterns_enables_show,
    .store =&synaptics_rmi4_glove_patterns_enables_store,
};

static struct kobj_attribute synaptics_firmware_version_attr = {
    .attr = {
        .name = "firmware_version",
        .mode = (S_IRUGO | S_IWUGO),
    },
    .show = &synaptics_rmi4_firmware_version_show,
    .store =&synaptics_rmi4_firmware_version_store,
};
...
struct kobject *properties_kobj_synap_hd;
struct kobject *properties_kobj_driver_hd;
...
#endif
#if 1
        __ATTR(glove_patterns_enables, (S_IRUGO | S_IWUSR | S_IWGRP),
                        synaptics_rmi4_glove_patterns_enables_show,
                        synaptics_rmi4_glove_patterns_enables_store),


        __ATTR(firmware_version, (S_IRUGO | S_IWUSR | S_IWGRP),
                        synaptics_rmi4_firmware_version_show,
                        synaptics_rmi4_firmware_version_store),
#endif
/*
                ...
                struct device_attribute *attr, char *buf)
{
	...
	unsigned int config_id_no = 0;
       ...
       retval = tpd_i2c_read_data_hd(ts->client,0x004c,config_id,sizeof(config_id));
	if (retval < 0)
	{
...
{
        int retval;
        unsigned char glove_patterns_enables;

        retval = tpd_i2c_read_data_hd(ts->client,
                        0x85,
                        &glove_patterns_enables,
                        sizeof(glove_patterns_enables));
        if (retval < 0) {
                dev_err(dev,
                                "%s: Failed to read glove patterns_enables enables, error = %d\n",
                                __func__, retval);
                return retval;
        }
                ...
                struct device_attribute *attr, const char *buf, size_t count)
{
        ...
        glove_patterns_enables = input;

        retval = tpd_i2c_write_data_hd(ts->client,
                        0x85,
                        &glove_patterns_enables,
                        sizeof(glove_patterns_enables));
        if (retval < 0) {
                dev_err(dev,
                                "%s: Failed to write glove_patterns_enables enables, error = %d\n",
                                __func__, retval);
                return retval;
        }
...
int tpd_i2c_read_data_hd(struct i2c_client *client,
...
EXPORT_SYMBOL(tpd_i2c_read_data_hd);

int tpd_i2c_write_data_hd(struct i2c_client *client,
                unsigned short addr, unsigned char *data, unsigned short length)
{
	u8 retval=0;
	u8 retry = 0;
...
EXPORT_SYMBOL(tpd_i2c_write_data_hd);
        ...
        for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
                for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
                        ...
                        case SYNAPTICS_RMI4_F11:
                                ...
                                retval = tpd_i2c_read_data_hd(ts->client,ts->f11.query_base,f11_query,sizeof(f11_query));
                                if (retval < 0)
                                        return retval;
                                ...
                                retval = tpd_i2c_read_data_hd(ts->client,ts->f11.ctrl_base+6, (char *)(&f11_max_xy), sizeof(f11_max_xy));
                                if (retval < 0)
                                        return retval;
...
{	
	...
	synaptics_fw_updater_hd(SynaFirmware_hd);
...
#endif
/**
* synaptics_rmi4_new_function_hd()
*
...
* respectively.
*/
void synaptics_rmi4_new_function_hd(enum exp_fn fn_type, bool insert,
...
EXPORT_SYMBOL(synaptics_rmi4_new_function_hd);
...
{       
	...
	if (tpd_halt)
	{
        ...
        //clear interrupt bit
        retval = tpd_i2c_read_data_hd(ts->client, ts->f01.data_base + 1, &status, 1);
        if (retval < 0)
	{
		mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
		continue;
        ...
        if (status & ts->fn1a_mask) {
                ...
                retval = tpd_i2c_read_data_hd(ts->client, 0x200, &button, 1);
                if (button)
                {
                       		...
                       		if (button & (0x01 << i))
				{
					...
					tpd_down(ppt_v.x, ppt_v.y, 20, 10);
				}
                        }
                }
        }
	...
	{
                tpd_i2c_read_data_hd(ts->client, ts->f11.data_base, finger_status_reg, (ts->points_supported + 3) / 4);
                ...
                for (i = 0; i < ts->points_supported; i++) 
		{
                        ...
                        if (0x01==finger_status || 0x02 == finger_status) 
			{
                                ...
                                tpd_i2c_read_data_hd(ts->client, ts->f11.data_base + num_of_finger_status_regs + i * ts->data_length,
                                                        data, ts->data_length);
	...
	mutex_unlock(&i2c_access);
	if(!finger_num&&button)tpd_down(ppt_v.x, ppt_v.y, 20, 10);
...
static int tpd_sw_power(struct i2c_client *client, int on)
{
        ...
        retval = tpd_i2c_read_data_hd(client,
                                ts->f01.ctrl_base,
                                &device_ctrl,
                                sizeof(device_ctrl));
        if (retval < 0) {
        ...
        if (on)
        {
                ...
                retval = tpd_i2c_write_data_hd(client,
                                ts->f01.ctrl_base,
                                &device_ctrl,
                                sizeof(device_ctrl));
                if (retval < 0) {
...
static int tpd_clear_interrupt(struct i2c_client *client)
{
        int retval = 0;
        u8 status = 0;
        retval = tpd_i2c_read_data_hd(client, ts->f01.data_base + 1, &status, 1);
        if (retval < 0){
                dev_err(&client->dev,
...
TPD_RESET_PROBE:
	...
	hwPowerOn(TPD_POWER_SOURCE_CUSTOM, VOL_2800, "TP");
        hwPowerOn(TPD_POWER_SOURCE_1800, VOL_1800, "CTP");
    	printk("WKP HD  power on ok\n");
	msleep(10);
        ...
        if (!i2c_check_functionality(client->adapter, I2C_FUNC_SMBUS_BYTE_DATA)) 
	{
        ...
        if (!ts) 
	{
                TPD_DMESG("WKP HD Failed to alloc mem for tpd_data\n");
                return -ENOMEM;
        }
        ...
        if (!td) 
	{
                TPD_DMESG("WKP HD Failed to alloc mem for tpd_debug\n");
        }

        ts->client = client;
        mutex_init(&(ts->io_ctrl_mutex));
		
	if((tpd_i2c_read_data_hd(ts->client, 0xEE, &data, 1))< 0)
	{	
		TPD_DMESG("WKP HD Can't connect touch panel.\n");
        ...
        if (retval < 0) 
	{
                TPD_DMESG("WKP HD Failed to query device\n");
	...
	properties_kobj_synap_hd = kobject_create_and_add("synapics", NULL);

#ifdef HAVE_TOUCH_KEY
	set_bit(EV_KEY, tpd->dev->evbit);
	for(i=0;i<MAX_KEY_NUM;i++)
		__set_bit(touch_key_array_hd[i], tpd->dev->keybit);
#endif
	...
	{
		TPD_DMESG("WKP HD auto upgrade\n");
		synaptics_rmi4_detection_work(NULL);
		synaptics_fw_updater_hd(SynaFirmware_hd);
	}
#endif
#if 0
	tpd_i2c_read_data_hd(client, ts->f11.ctrl_base+6, &tp_x_for_lcd,2);
	tpd_i2c_read_data_hd(client, ts->f11.ctrl_base+8,&tp_y_for_lcd,2);
	...
	if(tp_x_for_lcd!= (u16)TPD_RES_X||tp_y_for_lcd != (u16)TPD_RES_Y)
	{
		TPD_DMESG("Reset the resolution for LCM\n");
        	tpd_i2c_write_data_hd(client, ts->f11.ctrl_base+6, &(u16)TPD_RES_X,2);
        	tpd_i2c_write_data_hd(client, ts->f11.ctrl_base+8, &(u16)TPD_RES_Y,2);
	}
#endif	
        properties_kobj_driver_hd = kobject_create_and_add("driver", properties_kobj_synap_hd);

        for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) 
	{
                retval = sysfs_create_file(properties_kobj_driver_hd, &attrs[attr_count].attr);

                if (retval < 0) 
		{
                        dev_err(&client->dev, "%s:WKP HD Failed to create sysfs attributes\n", __func__);
                        goto err_sysfs;
                }
        }
	...
	if (IS_ERR(thread))
	{
		...
		TPD_DMESG("WKP HD failed to create kernel thread: %d\n", retval);
	...
	mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, EINTF_TRIGGER_FALLING, tpd_eint_handler, 0);
        ...
        tpd_load_status = 1;
        TPD_DMESG("%s:WKP HD TouchPanel Device Probe %s\n", __func__, (retval < 0) ? "FAIL" : "PASS");
...
//++++rgk bug_id: none add by jiangwanwei 140312 start++++
#if 1
	{
		...
		len = sprintf(tp_info, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,", "s3202", "XinLi", client->addr);
...
err_sysfs:
	...
	{
                sysfs_remove_file(properties_kobj_driver_hd, &attrs[attr_count].attr);
        }
...
static int tpd_local_init(void)
{
    ...
    {
    	TPD_DMESG("WKP HD Synaptics add error touch panel driver.\n");
...
#if 0	
	tpd_i2c_read_data_hd(ts->client, ts->f11.ctrl_base+6, &tp_x_for_lcd,2);
	tpd_i2c_read_data_hd(ts->client, ts->f11.ctrl_base+8,&tp_y_for_lcd,2);
	if(tp_x_for_lcd!= (u16)TPD_RES_X||tp_y_for_lcd != (u16)TPD_RES_Y)
	{
		TPD_DMESG("Reset the resolution for LCM\n");
        	tpd_i2c_write_data_hd(ts->client, ts->f11.ctrl_base+6, &TPD_RES_X,2);
        	tpd_i2c_write_data_hd(ts->client, ts->f11.ctrl_base+8, &TPD_RES_Y,2);
	}	
#endif	
#endif
        tpd_clear_interrupt(ts->client);
...
static void tpd_suspend(struct early_suspend *h)
{
	...
	mutex_unlock(&i2c_access);
       ...
       hwPowerDown(TPD_POWER_SOURCE_1800,  "CTP");
#endif
...
{
        TPD_DMESG("WKP HD Synaptics touch panel driver init\n");
        i2c_register_board_info(0, &i2c_tpd, 1);
        if(tpd_driver_add(&tpd_device_driver) < 0)
                TPD_DMESG("WKP HD Error Add Synaptics driver failed\n");
        return 0;
}
