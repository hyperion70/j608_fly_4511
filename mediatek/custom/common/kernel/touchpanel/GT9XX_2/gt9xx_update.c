...
#if ( (GTP_AUTO_UPDATE && GTP_HEADER_FW_UPDATE) || GTP_COMPATIBLE_MODE || defined(SELF_APK_UPGRADE) )
...
/*
	function : get file firmware version in firmware.h 
*/
void get_file_fw_version(u16 *file_fw_version)
{
    st_fw_head  file_fw_head;
	
	 if (CHIP_TYPE_GT9F == gtp_chip_type)
	 {
	 	memcpy(&file_fw_head, gtp_default_FW_fl, FW_HEAD_LENGTH);
	 }
	 else
	 {
        memcpy(&file_fw_head, gtp_default_FW, FW_HEAD_LENGTH);
	 }
      *file_fw_version = ((file_fw_head.vid & 0xFF00) >> 8) + ((file_fw_head.vid & 0x00FF) << 8);
}
//end
...
#if GTP_AUTO_UPDATE || defined(SELF_APK_UPGRADE) 
...
{
#if ((GTP_AUTO_UPDATE && GTP_HEADER_FW_UPDATE) || defined(SELF_APK_UPGRADE) )
