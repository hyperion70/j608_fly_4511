...
send_main_clk:
    ...
    //return SUCCESS;
exit_clk_proc:
//++++rgk add by wuhao start 2014/4/22
	if ((!IS_ERR(flp)) && (flp))
    {
 			filp_close(flp, NULL);
    }
	//++++rgk add by wuhao end 2014/4/22
    return ret;
}
   ...
   u8  rqst_data[3] = {(u8)(GTP_REG_RQST >> 8), (u8)(GTP_REG_RQST & 0xFF), 0};
    ...
    do
    {
        ...
        if (touch_num)
        {
            ...
            #endif
            GTP_DEBUG("*********** %d)(%d, %d)[%d], ### id :%d", id, input_x, input_y, input_w, sensor_id_g);
           //delete by wuhao (start)2014/1/15     
