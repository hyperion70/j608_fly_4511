...
extern  int  fix_tp_proc_info(void  *tp_data, u8 data_len);//add by xuhongming
...
static void gsl_hw_init(void)
{
	...
	hwPowerOn(MT6323_POWER_LDO_VIO28, VOL_1800, "TP");
	/* reset ctp gsl1680 */
...
static int __devinit gsl_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//add by xuhongming for 9375 mode TP information start
	int len;
	unsigned char tp_info[512];
	//add by xuhongming for 9375 mode TP information end
#ifdef TPD_PROXIMITY
	...
	//add by xuhongming for 9375 mode TP information start
	len=sprintf(tp_info,"TP IC:%s,TP MOUDLE:%s,TP I2C ADR:0x%x,FirmWare Version:0x%s","gsl915","TRUST",client->addr,"0x86ddb27a");
