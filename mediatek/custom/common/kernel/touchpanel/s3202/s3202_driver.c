...
static const struct i2c_device_id tpd_id[] = {{"s3202",0},{}};
unsigned short force[] = {0,(0x20<<1),I2C_CLIENT_END,I2C_CLIENT_END}; 
static const unsigned short * const forces[] = { force, NULL };
//static struct i2c_client_address_data addr_data = { .forces = forces, };
static struct i2c_board_info __initdata i2c_tpd={ I2C_BOARD_INFO("s3202", (0x20))};
...
static int tpd_i2c_write_byte_data(struct i2c_client *client, u8 command, u8 data)
{
	...
	printk("tpd_i2c_write_byte_data  command=%x,data = %x,ret=%d\r\n",command,data,ret);
...
static ssize_t update_firmware_show(struct kobject *kobj, struct kobj_attribute *attr,char *buf)
{
	int ret = i2c_smbus_read_byte_data(g_client, fd_34.controlBase+3);
	printk("The synaptics fw version is : 0x%x\n", ret);
...
{
	TPD_DMESG("s3202 I2C Touchscreen Driver (Built %s @ %s)\n", __DATE__, __TIME__);
...
{
	TPD_DMESG("s3202 touch panel driver init\n");
	i2c_register_board_info(0, &i2c_tpd, 1);
	if(tpd_driver_add(&tpd_device_driver) < 0)
		TPD_DMESG("Error Add s3202 driver failed\n");
	return 0;
}

static void __exit tpd_driver_exit(void)
{
	TPD_DMESG("s3202 touch panel driver exit\n");
...
MODULE_DESCRIPTION("Mediatek s3202 Driver");
