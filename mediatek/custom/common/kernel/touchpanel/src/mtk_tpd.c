...
//lisong 2014-3-10 [BUGID:NULL][fix bug:can not go into meta mode if TP disconnected ]start
#include <mach/mt_boot.h>
//lisong 2014-3-10 [BUGID:NULL][fix bug:can not go into meta mode if TP disconnected ]end
...
static int tpd_probe(struct platform_device *pdev) {
    ...
    int i=0;
#if 1//lisong 2014-3-10 [BUGID:NULL][fix bug:can not go into meta mode if TP disconnected ]start  
    int boot_mode = NORMAL_BOOT;
    boot_mode = get_boot_mode();
    printk("[tp_debug]:boot_mode=%d\n",boot_mode);
#endif//lisong 2014-3-10 [BUGID:NULL][fix bug:can not go into meta mode if TP disconnected ]end   
  ...
  TPD_RES_X = DISP_GetScreenWidth();
	...
	set_bit(KEY_O, tpd->dev->keybit);
	set_bit(KEY_LEFT, tpd->dev->keybit);
	set_bit(KEY_RIGHT, tpd->dev->keybit);
	set_bit(KEY_UP, tpd->dev->keybit);
	set_bit(KEY_DOWN, tpd->dev->keybit);
	set_bit(KEY_W, tpd->dev->keybit);
	set_bit(KEY_M, tpd->dev->keybit);
	set_bit(KEY_E, tpd->dev->keybit);
	set_bit(KEY_C, tpd->dev->keybit);
	set_bit(KEY_H, tpd->dev->keybit);
...
static int __init tpd_device_init(void) {
    ...
    rgk_creat_proc_tp_info();	//++++rgk bug_id: none add by jiangwanwei 140312++++
