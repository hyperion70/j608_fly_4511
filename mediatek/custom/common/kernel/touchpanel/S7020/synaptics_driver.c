...
#include "tpd_custom_s7020.h"//alpha.kong 20140226
#include "cust_gpio_usage.h"
#include "tpd.h"
#include "synaptics_dsx_rmi4_i2c.h"

#include "SynaImage.h"//alpha.kong 20140226 hd firmware
#include "SynaImage_s7020_hd.h"//alpha.kong 20140226 firmware
...
#define CONFIG_ID 0x30303043//alpha.kong modify 20140224
...
static struct i2c_board_info __initdata i2c_tpd={ I2C_BOARD_INFO(TPD_DEVICE, (0x20))};
                ...
                struct device_attribute *attr, char *buf)
{
	...
	retval = tpd_i2c_read_data(ts->client,0x004c,config_id,sizeof(config_id));
                ...
                struct device_attribute *attr, char *buf)
{
        ...
        retval = tpd_i2c_read_data(ts->client,
                        0x400,//alpha.kong modify 20140224
...
//alpha.kong modify 20140224 -s		
		if(glove_patterns_enables == 0x30)
		{
			glove_patterns_enables = 0x02;
		}
		else if(glove_patterns_enables == 0x32)
        {
            glove_patterns_enables = 0x01;
        }
//alpha.kong modify 20140224 -e
...
TPD_RESET_PROBE:
        ...
        if (!i2c_check_functionality(client->adapter, I2C_FUNC_SMBUS_BYTE_DATA)) 
	{
        ...
        if (!ts) 
	{
                TPD_DMESG("WKP Failed to alloc mem for tpd_data\n");
                return -ENOMEM;
        }
        ...
        if (!td) 
	{
                TPD_DMESG("WKP Failed to alloc mem for tpd_debug\n");
        }
	...
	{	
		TPD_DMESG("WKP Can't connect touch panel.\n");
        ...
        if (retval < 0) 
	{
                TPD_DMESG("WKP Failed to query device\n");
	...
	{
		TPD_DMESG("WKP auto upgrade\n");
		synaptics_rmi4_detection_work(NULL);

		//alpha.kong 20140226 -s
		if((DISP_GetScreenHeight()== 1280)&&(DISP_GetScreenWidth()== 720))//hd
		{
			synaptics_fw_updater(SynaFirmware_s7020_hd);
		}
		else//full hd 1920*1080
		{
			synaptics_fw_updater(SynaFirmware);
		}
		//alpha.kong 20140226 -e
        ...
        for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) 
	{
                ...
                if (retval < 0) 
		{
                        dev_err(&client->dev, "%s:WKP Failed to create sysfs attributes\n", __func__);
                        goto err_sysfs;
                }
        }
	...
	if (IS_ERR(thread))
	{
		...
		TPD_DMESG("WKP failed to create kernel thread: %d\n", retval);
	...
	mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, EINTF_TRIGGER_FALLING, tpd_eint_handler, 0);
        ...
        tpd_load_status = 1;
        TPD_DMESG("%s:WKP TouchPanel Device Probe %s\n", __func__, (retval < 0) ? "FAIL" : "PASS");
    ...
    {		
       ...
       len = sprintf(tp_info, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,", "s7020", "XinLi", client->addr);		
...
//alpha.kong 20140226 -s	
#ifdef TPD_HAVE_BUTTON
	if((DISP_GetScreenHeight()== 1280)&&(DISP_GetScreenWidth()== 720))
	{
	
		int tpd_keys_dim_local_wvga_new[TPD_KEY_COUNT][4]=TPD_KEYS_DIM_HD;
		
		memcpy(tpd_keys_dim_local_wvga, tpd_keys_dim_local_wvga_new, TPD_KEY_COUNT*4*4);
...
//alpha.kong 20140226 -e
    ...
    {
    	TPD_DMESG("WKP Synaptics add error touch panel driver.\n");
...
{
        TPD_DMESG("WKP Synaptics touch panel driver init\n");
        i2c_register_board_info(0, &i2c_tpd, 1);
        if(tpd_driver_add(&tpd_device_driver) < 0)
                TPD_DMESG("WKP Error Add Synaptics driver failed\n");
        return 0;
}
