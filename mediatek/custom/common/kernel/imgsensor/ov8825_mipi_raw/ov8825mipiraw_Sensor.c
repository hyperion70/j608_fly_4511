...
//#define OTP_OV8825        //currently there is no otp
#ifdef OTP_OV8825
static int  stmid = 0;
#endif
#define BG_Ratio_Typical 0x55
#define RG_Ratio_Typical 0x4b
...
printk("check_otp_wb before flag=0x%x\n",flag);
flag = flag & 0x80;
printk("check_otp_wb after flag=0x%x\n",flag);

rg = OV8825_read_cmos_sensor(address+2);
printk("check_otp_wb rg=0x%x\n",rg);

bg = OV8825_read_cmos_sensor(address+3);
printk("check_otp_wb bg=0x%x\n",bg);
...
int address;
bank = 0x08 | (index * 2 - 1);
OV8825_write_cmos_sensor(0x3d84, bank);
OV8825_write_cmos_sensor(0x3d81, 0x01);
msleep(20);
address = 0x3d00;
flag = OV8825_read_cmos_sensor(address);
printk("check_otp_lenc  flag=0x%x\n",flag);
flag = flag & 0xc0;
for (i=0;i<32;i++) {
OV8825_write_cmos_sensor(0x3d00 + i, 0x00);
}
...
int i;
int address;
OV8825DB("sym read_otp_wb  \n");
OV8825_write_cmos_sensor(0x3d84, 0x08);
OV8825_write_cmos_sensor(0x3d81, 0x01);
msleep(20);
address = 0x3d05 + (index-1)*9;
(*otp_ptr).module_integrator_id = OV8825_read_cmos_sensor(address) & 0x7f;
(*otp_ptr).lens_id = OV8825_read_cmos_sensor(address + 1);
(*otp_ptr).rg_ratio = OV8825_read_cmos_sensor(address + 2);
(*otp_ptr).bg_ratio = OV8825_read_cmos_sensor(address + 3);
(*otp_ptr).user_data[0] = OV8825_read_cmos_sensor(address + 4);
(*otp_ptr).user_data[1] = OV8825_read_cmos_sensor(address + 5);
(*otp_ptr).user_data[2] = OV8825_read_cmos_sensor(address + 6);
(*otp_ptr).light_rg = OV8825_read_cmos_sensor(address + 7);
(*otp_ptr).light_bg = OV8825_read_cmos_sensor(address + 8);
stmid = (*otp_ptr).module_integrator_id;
OV8825DB("sym read_otp_wb MID = %d\n",stmid);
for (i=0;i<32;i++) {
OV8825_write_cmos_sensor(0x3d00 + i, 0x00);
}
return 0;
...
address = 0x3d01;
for(i=0;i<31;i++) {
(* otp_ptr).lenc[i]=OV8825_read_cmos_sensor(address);
address++;
}
for (i=0;i<32;i++) {
OV8825_write_cmos_sensor(0x3d00 + i, 0x00);
...
{
int i, temp;
temp = OV8825_read_cmos_sensor(0x5000);
temp = 0x80 | temp;
OV8825_write_cmos_sensor(0x5000, temp);
for(i=0;i<62;i++) {
OV8825_write_cmos_sensor(0x5800 + i, (*otp_ptr).lenc[i]);
}
return 0;
}
int update_otp_wb()
...
for(i=1;i<=3;i++) {
temp = check_otp_wb(i);
printk("sym update_otp_wb temp=%d\n",temp);
if (temp == 2) {
otp_index = i;
break;
}
...
printk("sym update_otp_lenc temp=%d\n",temp);
if (temp == 2) {
otp_index = i;
break;
}
...
UINT32 OV8825Open(void)
{
  ...
  mdelay(50);
#ifdef OTP_OV8825
	OV8825DB("OV8825_Sensor_Init update_otp_lenc /update_otp_wb  aaaa:\n ");
	update_otp_lenc();
	update_otp_wb();
	if(0x08 != stmid)
	{
