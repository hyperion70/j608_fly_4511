...
void GC0329NightMode(kal_bool bEnable)
{
	...
	else 
	{
		...
		GC0329_write_cmos_sensor(0xfe, 0x00);
		GC0329GammaSelect(GC0329_RGB_Gamma_m4);//becky  GC0329GammaSelect(GC0329_RGB_Gamma_m6)
...
void GC0329_Sensor_Init(void)
{
	...
	GC0329_write_cmos_sensor(0x20, 0x01);  // 00 tao 2.22
	...
	GC0329_write_cmos_sensor(0x82, 0x9f); //55  5f becky
	...
	////////////////////YCP////////////////////
	GC0329_SET_PAGE0;
	GC0329_write_cmos_sensor(0xd1, 0x28);//30 becky
	GC0329_write_cmos_sensor(0xd2, 0x28);
	GC0329_write_cmos_sensor(0xd3, 0x40);//38
	...
	GC0329_write_cmos_sensor(0x13, 0x54);//becky 50
...
BOOL GC0329_set_param_wb(UINT16 para)
{
	...
	switch (para)
	{
		...
		case AWB_MODE_CLOUDY_DAYLIGHT: //cloudy
			...
			GC0329_write_cmos_sensor(0x77, 0xb0); //WB_manual_gain 8c becky  90  70
			GC0329_write_cmos_sensor(0x78, 0x70);//50  60    58
			GC0329_write_cmos_sensor(0x79, 0x38);//40  3a    30
		break;
		
		case AWB_MODE_DAYLIGHT: //sunny
			GC0329_awb_enable(KAL_FALSE);
			GC0329_write_cmos_sensor(0x77, 0x90); //74  7a
			GC0329_write_cmos_sensor(0x78, 0x60);//52   58
			GC0329_write_cmos_sensor(0x79, 0x3a);//40			
		break;
		
		case AWB_MODE_INCANDESCENT: //office
			GC0329_awb_enable(KAL_FALSE);
			GC0329_write_cmos_sensor(0x77, 0x50);//  48 becky
			GC0329_write_cmos_sensor(0x78, 0x5a);//40
			GC0329_write_cmos_sensor(0x79, 0x52);//5c
		break;
		
		case AWB_MODE_TUNGSTEN: //home
			GC0329_awb_enable(KAL_FALSE);
			GC0329_write_cmos_sensor(0x77, 0x42);// becky 40  46
			GC0329_write_cmos_sensor(0x78, 0x58);//54  62
			GC0329_write_cmos_sensor(0x79, 0x68);//70
		break;
		
		case AWB_MODE_FLUORESCENT:
			GC0329_awb_enable(KAL_FALSE);
			GC0329_write_cmos_sensor(0x77, 0x58);//40  
			GC0329_write_cmos_sensor(0x78, 0x4e);//42
			GC0329_write_cmos_sensor(0x79, 0x40);//50
		break;
...
BOOL GC0329_set_param_effect(UINT16 para)
{
	...
	switch (para)
	{
		...
		case MEFFECT_SEPIA:
			...
			GC0329_write_cmos_sensor(0xda , 0xcd);//d0 e0
			GC0329_write_cmos_sensor(0xdb , 0x24);//28
		break;
...
BOOL GC0329_set_param_banding(UINT16 para)
{
	switch (para)
	{
		case AE_FLICKER_MODE_50HZ:
			...
			GC0329_SET_PAGE1;
			GC0329_write_cmos_sensor(0x29, 0x00);   //anti-flicker step [11:8]
			GC0329_write_cmos_sensor(0x2a, 0x78);    //anti-flicker step [7:0] 3c
			
			GC0329_write_cmos_sensor(0x2b, 0x03);   //exp level 0  10fps  0258
			GC0329_write_cmos_sensor(0x2c, 0x48); 
			GC0329_write_cmos_sensor(0x2d, 0x02);   //exp level 1  12.50fps
			GC0329_write_cmos_sensor(0x2e, 0xd0); 
			GC0329_write_cmos_sensor(0x2f, 0x04);   //exp level 2  10.00fps
			GC0329_write_cmos_sensor(0x30, 0x80); 
			GC0329_write_cmos_sensor(0x31, 0x04);   //exp level 3  7.14fps
			GC0329_write_cmos_sensor(0x32, 0xb0); 
			GC0329_SET_PAGE0;
			break;
...
BOOL GC0329_set_param_exposure(UINT16 para)
{
	...
	value_Y = (GC0329_NIGHT_MODE?0x68:0x5a);   // 50 tao 2.22
