...
//	GSE_FUN();
	...
	{ 
		...
		databuf1[0] = 0x01;//0x28;// CYCLE =1 ;SLEEP =0; TEMP_DIS =1; internal 8M clock
...
static int MPU6050C_ACC_SetAccDataRange(struct i2c_client *client, u8 range)
{
	...
	if(res <= 0)
	{
        GSE_ERR("MPU6050C_ACC_SetAccDataRange i2c send failed!\n");
		return MPU6050C_ACC_ERR_I2C;
	}
...
#if 1//lisong add for direction debug start
...
static int mpu6050c_acc_init_client(struct i2c_client *client, bool enable)
{
	...
	if(res != MPU6050C_ACC_SUCCESS)
	{
        GSE_ERR("MPU6050C_ACC_SetPWR_MGMT_12 failed!\n");
		return res;
	}
...
//				GSE_ERR("X :%d,Y: %d, Z: %d\n",acc_data->values[0],acc_data->values[1],acc_data->values[2]);
			}
			break;
		default:
