...
#define MMC3416X_DEBUG_MSG	0
#define MMC3416X_DEBUG_FUNC	0
...
static int mmc3416x_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	{
		MMCDBG("mmc3416x: read power ctl register err and retry!\n");
		if(hwmsen_read_byte(client, addr, databuf))
	    {
		   MMCDBG("mmc3416x: read power ctl register retry err!\n");
		   return -1;
	    }
	}
	...
	if(res <= 0)
	{
		MMCDBG("mmc3416x: set power mode failed!\n");
		return -1;
	}
	else
	{
		MMCDBG("mmc3416x: set power mode ok %x!\n", databuf[1]);
	}
...
static void mmc3416x_power(struct mag_hw *hw, unsigned int on) 
{
	...
	{        
		...
		else if(on)
		{
			...
			{
				MMCDBG(KERN_ERR "power on fails!!\n");
			}
		}
		else
		{
			...
			{
				MMCDBG(KERN_ERR "power off fail!!\n");
			}
		}
	}
	power_on = on;
...
static int I2C_RxData(char *rxData, int length)
{
	...
	{
		MMCDBG(KERN_ERR "%s retry over %d\n", __func__, MMC3416X_RETRY_COUNT);
		return -EIO;
	}
#if DEBUG
...
static int ECS_GetRawData(int data[3])
{
	...
	if(err !=0 )
	{
		MMCDBG(KERN_ERR "MMC3416X_IOC_TM failed\n");
		return -1;
	}
...
static ssize_t store_layout_value(struct device_driver *ddri, char *buf, size_t count)
{
	...
	if(1 == sscanf(buf, "%d", &layout))
	{
		...
		{
			MMCDBG(KERN_ERR "HWMSEN_GET_CONVERT function error!\r\n");
		}
		else if(!hwmsen_get_convert(data->hw->direction, &data->cvt))
		{
			MMCDBG(KERN_ERR "invalid layout: %d, restore to %d\n", layout, data->hw->direction);
		}
		else
		{
			MMCDBG(KERN_ERR "invalid layout: (%d, %d)\n", layout, data->hw->direction);
			hwmsen_get_convert(0, &data->cvt);
		}
	}
	else
	{
		MMCDBG(KERN_ERR "invalid format = '%s'\n", buf);
	}
...
static ssize_t show_trace_value(struct device_driver *ddri, char *buf)
{
	...
	if(NULL == obj)
	{
		MMCDBG(KERN_ERR "mmc3416x_i2c_data is null!!\n");
		return 0;
	}	
...
static ssize_t store_trace_value(struct device_driver *ddri, char *buf, size_t count)
{
	...
	{
		MMCDBG(KERN_ERR "invalid content: '%s', length = %d\n", buf, count);
	}
...
static int mmc3416x_create_attr(struct device_driver *driver) 
{
	...
	for(idx = 0; idx < num; idx++)
	{
		...
		{            
			MMCDBG(KERN_ERR "driver_create_file (%s) = %d\n", mmc3416x_attr_list[idx]->attr.name, err);
			break;
		}
	}
...

{
	...
	switch (cmd)
	{
		...
		case MMC31XX_IOC_SET:
			...
			if(I2C_TxData(data, 2) < 0)
			{
				MMCDBG(KERN_ERR "MMC3416X_IOC_SET failed\n");
				return -EFAULT;
			}
			...
			data[1] = MMC3416X_CTRL_SET;
			if (I2C_TxData(data, 2) < 0) {
	                MMCDBG(KERN_ERR "MMC3416X_IOC_SET failed2\n");
				return -EFAULT;
			}
			msleep(1);
			data[0] = MMC3416X_REG_CTRL;
			data[1] = 0;
			if (I2C_TxData(data, 2) < 0) {
	                MMCDBG(KERN_ERR "MMC3416X_IOC_SET failed3\n");
				return -EFAULT;
			}
			msleep(MMC3416X_DELAY_SET);
			break;
		...
		case MMC31XX_IOC_RESET:
			...
			if(I2C_TxData(data, 2) < 0)
			{
				MMCDBG(KERN_ERR "MMC3416X_IOC_RESET failed\n");
				return -EFAULT;
			}
			/* wait external capacitor charging done for next SET/RESET */
			msleep(MMC3416X_DELAY_RST);
			...
			data[1] = MMC3416X_CTRL_RESET;
			if (I2C_TxData(data, 2) < 0) {
	                MMCDBG(KERN_ERR "MMC3416X_IOC_RESET failed2\n");
				return -EFAULT;
			}
			msleep(1);
			data[0] = MMC3416X_REG_CTRL;
			data[1] = 0;
			if (I2C_TxData(data, 2) < 0) {
	               MMCDBG(KERN_ERR "MMC3416X_IOC_RESET failed3\n");
				return -EFAULT;
			}
			msleep(1);
			break;
		...
		case MMC31XX_IOC_READ:
			...
			if(I2C_RxData(data, 6) < 0)
			{
				MMCDBG(KERN_ERR "MMC3416X_IOC_READ failed\n");
				return -EFAULT;
			}
			vec[0] = data[1] << 8 | data[0];
			vec[1] = data[3] << 8 | data[2];
			...
			{
				MMCDBG(KERN_ERR "MMC3416X_IOC_READ: copy to user failed\n");
				return -EFAULT;
			}
			break;
		...
		case MMC31XX_IOC_READXYZ:
			...
			if(copy_to_user(argp, vec, sizeof(vec)))
			{
				MMCDBG(KERN_ERR "MMC3416X_IOC_READXYZ: copy to user failed\n");
				return -EFAULT;
			}
			break;			
		...
		case ECOMPASS_IOC_GET_DELAY:			
			...
			{
				MMCDBG(KERN_ERR "copy_to_user failed.");
				return -EFAULT;
			}
			break;		
		...
		case MSENSOR_IOCTL_READ_CHIPINFO:
			if(argp == NULL)
			{
				MMCDBG(KERN_ERR "IO parameter pointer is NULL!\r\n");
				break;
			}
		...
		default:
			MMCDBG(KERN_ERR "%s not supported = 0x%04x", __FUNCTION__, cmd);
			return -ENOIOCTLCMD;
			break;		
		}
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			break;
		default:
			MMCDBG(KERN_ERR "msensor operate function no this parameter %d!\n", command);
			err = -1;
			break;
	}
...
static void mmc3416x_early_suspend(struct early_suspend *h) 
{
	...
	if(NULL == obj)
	{
		MMCDBG(KERN_ERR "null pointer!!\n");
		return;
	}
	if(mmc3416x_SetPowerMode(obj->client, false))
	{
		MMCDBG("mmc3416x: write power control fail!!\n");
		return;
	}
...
//modified for mmc3416x----changhua start
	...
	{
		MMCDBG(KERN_ERR "mmc3416x_device set ST cmd failed\n");
		goto exit_kfree;
	}
#endif	
	...
	{
		MMCDBG(KERN_ERR "create attribute err = %d\n", err);
	...
	{
		MMCDBG(KERN_ERR "mmc3416x_device register failed\n");
...
static int mmc3416x_i2c_remove(struct i2c_client *client)
{
	...
	{
		MMCDBG(KERN_ERR "mmc3416x_delete_attr fail: %d\n", err);
	}
...
static int mmc_probe(struct platform_device *pdev) 
{
	...
	{
		MMCDBG(KERN_ERR "add driver error\n");
		return -1;
	} 
	return 0;
}
...
static int __init mmc3416x_init(void)
{
	...
	{
		MMCDBG(KERN_ERR "failed to register driver");
		return -ENODEV;
	}
	return 0;    
}
