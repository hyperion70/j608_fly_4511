...
/********************************************************************/
//add by yangjuwei
static ssize_t cm36283_ps_store_canc(struct device_driver *ddri, const char *buf, size_t count)
{	
	u8 databuf[3];
	int res;
	...
	read_int_from_buf(cm36283_obj, buf, count, &databuf[1], 1);
		
	if(databuf[1] >= 50)          //add a MAX Calibration value for error writing
		databuf[1] = 0;

	databuf[2]= 0;	

	res = CM36283_i2c_master_operate(cm36283_obj->client,databuf, 0x3, I2C_FLAG_WRITE);
	if(res <= 0)
	{
		APS_ERR("i2c_master_send function err\n");
	}
...
/********************************************************************/
//add by yangjuwei
static ssize_t cm36283_ps_show_canc(struct device_driver *ddri, char *buf)
{	
	u8 databuf[3];
	int res;
	if(!cm36283_obj)
	...
	//printk("read canc = %d,cm36283_obj->ps_canc = %d\n",databuf[0],cm36283_obj->ps_canc);
	//cm36283_enable_ps(cm36283_obj->client, 0);
	
	res = snprintf(buf, PAGE_SIZE,"read ps_canc = %d\n", databuf[0]);
	
	return res;
}

static DRIVER_ATTR(canc,   S_IWUSR | S_IRUGO, cm36283_ps_show_canc, cm36283_ps_store_canc);//add by yangjuwei
/*---------------------------------------------------------------------------------------*/
...
static struct driver_attribute *cm36283_attr_list[] = {
    ...
    &driver_attr_recv,
    &driver_attr_reg,
	&driver_attr_canc,//add by yangjuwei
};
...
static long cm36283_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
		...
		switch (cmd)
		{
			...
			case ALSPS_GET_PS_TEST_RESULT:
				...
				#if 0 //+++rgk anxiang.xiao change for cit pass norally 20140414
