...
#ifdef RGK_KEY_VOLUME_MUTE//lisong 2014-6-10 [NO BUGID][config key for volume mute]start
static void kpd_key_mute_handler(unsigned long data);
static void kpd_key_unmute_handler(unsigned long data);
static DECLARE_TASKLET(kpd_key_mute_tasklet, kpd_key_mute_handler, 0);
static DECLARE_TASKLET(kpd_key_unmute_tasklet, kpd_key_unmute_handler, 0);
#endif//lisong 2014-6-10 [NO BUGID][config key for volume mute]end
...
static void kpd_key_mute_handler(unsigned long data)
{
    input_report_key(kpd_input_dev, RGK_KEY_MUTE, 1);
	input_sync(kpd_input_dev);
    input_report_key(kpd_input_dev, RGK_KEY_MUTE, 0);
	input_sync(kpd_input_dev);
    mt_eint_unmask(CUST_EINT_KEY_UNMUTE_NUM); 
    
}
static void kpd_key_unmute_handler(unsigned long data)
{
    input_report_key(kpd_input_dev, RGK_KEY_UNMUTE, 1);
	input_sync(kpd_input_dev);
    input_report_key(kpd_input_dev, RGK_KEY_UNMUTE, 0);
	input_sync(kpd_input_dev);
    mt_eint_unmask(CUST_EINT_KEY_MUTE_NUM); 
}

static void key_mute_eint_handler(void)
{
    tasklet_schedule(&kpd_key_mute_tasklet);    
}

static void key_unmute_eint_handler(void)
{
    tasklet_schedule(&kpd_key_unmute_tasklet);    
}

static void rgk_mute_key_setup_eint(void)
{
    /*key mute*/
    mt_set_gpio_mode(GPIO_KEY_MUTE_EINT_PIN, GPIO_KEY_MUTE_EINT_PIN_M_EINT);
    mt_set_gpio_dir(GPIO_KEY_MUTE_EINT_PIN, GPIO_DIR_IN);
    mt_set_gpio_pull_enable(GPIO_KEY_MUTE_EINT_PIN, GPIO_PULL_ENABLE);
    mt_set_gpio_pull_select(GPIO_KEY_MUTE_EINT_PIN, GPIO_PULL_UP);

	mt_eint_set_hw_debounce(CUST_EINT_KEY_MUTE_NUM, CUST_EINT_KEY_MUTE_DEBOUNCE_CN);
	mt_eint_registration(CUST_EINT_KEY_MUTE_NUM, CUST_EINT_KEY_MUTE_TYPE,key_mute_eint_handler, 0);
    mt_eint_unmask(CUST_EINT_KEY_MUTE_NUM); 
    
    /*key unmute*/
    mt_set_gpio_mode(CUST_EINT_KEY_UNMUTE_NUM, GPIO_KEY_UNMUTE_EINT_PIN_M_EINT);
    mt_set_gpio_dir(CUST_EINT_KEY_UNMUTE_NUM, GPIO_DIR_IN);
    mt_set_gpio_pull_enable(CUST_EINT_KEY_UNMUTE_NUM, GPIO_PULL_ENABLE);
    mt_set_gpio_pull_select(CUST_EINT_KEY_UNMUTE_NUM, GPIO_PULL_UP);

	mt_eint_set_hw_debounce(CUST_EINT_KEY_UNMUTE_NUM, CUST_EINT_KEY_UNMUTE_DEBOUNCE_CN);
	mt_eint_registration(CUST_EINT_KEY_UNMUTE_NUM, CUST_EINT_KEY_UNMUTE_TYPE,key_unmute_eint_handler, 0);
    mt_eint_unmask(CUST_EINT_KEY_UNMUTE_NUM); 
}
#endif//lisong 2014-6-10 [NO BUGID][config key for volume mute]end
...
#ifdef RGK_KEY_VOLUME_MUTE//lisong 2014-6-10 [NO BUGID][config key for volume mute]start
    __set_bit(RGK_KEY_MUTE, kpd_input_dev->keybit);
    __set_bit(RGK_KEY_UNMUTE, kpd_input_dev->keybit);
#endif//lisong 2014-6-10 [NO BUGID][config key for volume mute]end
