...
#if 1//lisong 2014-4-16 [NO BUGID][charging can not be stopped when battery full]start
#ifdef HIGH_BATTERY_VOLTAGE_SUPPORT
    if((BMT_status.ICharging <= charging_full_current)||(BMT_status.bat_vol >= 4350))
#else
    if((BMT_status.ICharging <= charging_full_current)||(BMT_status.bat_vol >= 4200))
#endif
#else
	if(BMT_status.ICharging <= charging_full_current)
#endif//lisong 2014-4-16 [NO BUGID][charging can not be stopped when battery full]end     
    {
    	full_check_count++;
...
extern kal_uint32 g_batt_temp_status;//Mr.Xu Change
...
/* dongwei.cao START */
    if ( (BMT_status.temperature <= MAX_CHARGE_TEMPERATURE_MINUS_X_DEGREE)
		...
		&& (g_batt_temp_status == TEMP_POS_HIGH)			//Mr.Xu add
